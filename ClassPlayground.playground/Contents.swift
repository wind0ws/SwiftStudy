...
/**
 ...
 便利构造器是类中比较次要的／辅助型的构造器。你可以定义便利构造器来调用同一个类中的指定构造器，并为其参数提供默认值。你可以定义便利构造器创建一个特殊用途或特定输入的实例。
 应当只有在必要的时候为类提供便利构造器，比方说某种情况下通过使用便利构造器来快捷调用某个指定构造器，能够节省更多开发时间并让类的构造过程更清晰明了。
 convenience init(parameters){
    statements
 }
 
规则：指定构造器是向上代理。 便利构造器是横向代理。
 
 两段式构造过程：1、每个存储属性通过引入他们的类的构造器来设置初始值。
                2、他给每个类一次机会在新实例准备使用之前进一步定制他们的存储型属性
